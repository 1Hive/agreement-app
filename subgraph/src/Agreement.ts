...
export function handleSettingChanged(event: SettingChanged): void {
  ...
  if (settingData.reverted) {
    log.error("Agrement app with corrupted setings {} ", [
  ...
  const currentVersionId = buildVersionId(
    event.address,
...
export function handleDisputableAppActivated(
  ...
  const agreementApp = AgreementContract.bind(event.address);
  const disputableData = agreementApp.getDisputableInfo(
...
export function handleActionClosed(event: ActionClosed): void {
  ...
  action.closed = true;
  action.save();
  const lastChallenge = action.lastChallenge;
  if (lastChallenge) {
    const challenge = Challenge.load(lastChallenge);
    if (challenge && challenge.state != "Settled") {
...
function updateCollateralRequirement(
  ...
  const token = buildERC20(agreement, requirementData.value0);
  if (token) {
...
function createArbitratorFee(
  ...
  const token = buildERC20(agreement, feeToken);
  if (token) {
